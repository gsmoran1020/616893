...
def seed():
    ...
    messages = Message(
        conversation=santiagoConvo, senderId=santiago.id, text="Where are you from?", messageRead=True
    )
    messages.save()

    messages = Message(
        conversation=santiagoConvo, senderId=thomas.id, text="I'm from New York", messageRead=True
    )
    messages.save()
